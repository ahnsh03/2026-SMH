...
**코스 ↔ 색 (SSOT):** **Out 코스 = 흰 차선**, **In 코스 = 노란 차선**.  
Out의 S자·갈림길·공통 구간은 흰 경계를 추종하고, In의 회전교차로(진입·원호·탈출)는 노란 경계를 추종한다.  
인지·플래너·spawn 검증이 이 계약을 공유한다 → [lane-occlusion-fork-strategy.md §0.1](./lane-occlusion-fork-strategy.md) · [competition.md](./competition.md).
