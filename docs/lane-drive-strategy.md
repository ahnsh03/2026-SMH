...
## 0. 개정 요약 (2026-07-12 control bridge)

| 항목 | 상태 |
|------|------|
| 인지/제어 | `inference_node` → `/perception/lane` → `lane_control_node` → `/control` |
| BEV 런타임 | **Metric IPM** (`lane_vision.yaml` `metric_ipm:`) — 사다리꼴은 참고 툴만 |
| `VISUALIZE` | 기본 **False** (`LANE_VISUALIZE=1`로만 ON) |
| 임시 조향 | `lane_planner` **Pure Pursuit** + EMA/rate (시뮬=LIMO 기하) |
| 타입 SSOT | `inference.types.LaneDetections` + `lane_adapters` |
| 시뮬 vs 실차 | 토픽·YAML 동일. 시뮬만 `STEER_TRIM=0` + `use_sim_time` |
| 안전 | `/perception/lane` timeout → throttle 0 |

원태 `detect()`에는 조향을 넣지 않는다.  
**팀원 PR 전 필독:** [lane-perception-topic.md](./lane-perception-topic.md) (시뮬/실차·체크리스트).
...
**런타임 (ROS launch — 토픽 분리):**

```
/camera → inference_node (lane_detection + Metric IPM)
       → /perception/lane
       → lane_control_node (adapter + lane_planner Pure Pursuit)
       → /control
```

**단프로세스/테스트:** `pipeline.run_perception` → adapter → `lane_planner` → `fuse_control`
...
| 계층 | 1차 채택 | 담당 | 비고 |
|------|----------|------|------|
| ROI/IPM 튜닝 | **Metric IPM** YAML (`y_half=0.77`) · 사다리꼴은 참고 | 원태 파이프라인 + 승현 툴 | §4.3 잠정 확정 · **런타임도 Metric IPM** |
| 런타임 인지 | 원태 `LaneDetections` → `/perception/lane` | 장원태 | `VISUALIZE` 기본 False |
| 경로·조향 | `lane_control_node` + **Pure Pursuit** (LIMO sim) | 안승현 | 미션 FSM·라운드어바웃은 양서준과 `/control` 합의 |
| 다중 경로 | 후보 N개 + 모드 | 안승현 (+원태 분기) | `fork_active`/`branches` 전달됨 |
| 종방향 | cruise × throttle_scale | 안승현 | |
...
인지 전용 `LaneDetections`. **런타임 BEV = Metric IPM.** 조향은 `lane_control_node`. `VISUALIZE` 기본 False (`LANE_VISUALIZE=1`). `types`+어댑터로 planner 연동 완료. 동일색 이중 분기는 Phase 3.
