...
> 작성: 2026-07-11 (안승현) · **개정: 2026-07-11** (원태 `0cbcdbe` 반영)  
...
### 4.4 트랙바 파라미터 (Phase 0)

| 파라미터 | 의미 |
|----------|------|
| `crop_top_ratio` | 상단 제외 비율 |
| `bottom_half_width_ratio` | 아랫변 반폭/(W/2). 1.0=풀폭, >1.0=바깥 확장 |
| `bev_width`, `bev_height` | BEV 해상도 |

**UI:** 원본 · ROI 오버레이 · BEV. `s` 저장, `q` 종료.
...
| Phase | 내용 | 산출물 | 상태 |
|-------|------|--------|------|
| **0** | ROI/IPM 튜닝 + 캡처 | `scripts/vision_tune/`, `config/lane_vision.yaml` | ← 진행 |
