...
| 주행 조향 | `pipeline.MainPlanner`의 PP + heading + CTE, 곡률 적응형 LD/속도 |
| 인지 타입 | 모듈 반환값 직접 사용, ROS msg/adapter는 외부 검증 호환용 |
| 시뮬 vs 실차 | 토픽·YAML 동일. 시뮬만 `STEER_TRIM=0` + `use_sim_time` |
| 안전 | 카메라 watchdog → steering/throttle 0 |
...
| 경로·조향 | `pipeline.MainPlanner` + **Pure Pursuit** | 양서준 | 일반·분기·회전교차로 FSM과 최종 `/control` 소유 |
