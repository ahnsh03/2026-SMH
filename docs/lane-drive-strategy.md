...
| **1** | HSV 정밀 튜닝 · 클릭 피커 | **장원태** | `hsv.white`/`yellow` · (원태 툴) | 대기 |
...
> Phase 1 HSV **정밀값**은 원태. 승현은 HSV에 막히지 않고 Phase 2(흰 L/R → 조향)를 먼저 닫는다.  
...
| 경로 | 역할 |
|------|------|
| `scripts/vision_tune/` | IPM·캡처·**제어 게인** 트랙바 |
...
4. **순서:** Phase 0 → **흰 planner + 제어 튜너(승현)** ∥ HSV 정밀(원태) → 노란·다중경로 → (선택) PP/Stanley  
5. **승현 튜너 1차:** 제어 게인 (`tune_lane_control`). HSV 정밀값은 원태.  
