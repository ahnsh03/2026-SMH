...
## 5.9 프로파일 — 같은 코드, 다른 차량

시뮬과 실차는 **코드를 분리하지 않는다.** `config/main_planner.yaml` 하단의
`profiles:` 블록이 베이스 섹션 위에 deep-merge 된다.

| | 프로파일 | 값 |
|---|---|---|
| Gazebo | `sim` | **오버라이드 없음** — 위쪽 베이스 섹션이 곧 시뮬 값이다 |
| D3-G 보드 | `real` | 실측 기하 + 2.2 Hz 인식 주기에 맞춘 타이밍 |

런치가 자동으로 선택한다:
- `src/inference/launch/auto_driving.launch.py` → `planner_profile: 'real'`
- `src/dracer_sim/launch/sim_auto_driving.launch.py` → `planner_profile: 'sim'`

`profiles.sim`이 비어 있으므로 **프로파일 없이 로드한 것과 `sim`은 완전히 동일하다**
(테스트로 `base == sim` 검증). 즉 실차 패치가 시뮬 개발에 아무 영향을 주지 않는다.

`real`이 덮어쓰는 값 (근거는 각각 5.1~5.4):

| 키 | sim | real | 이유 |
|---|---|---|---|
| `wheelbase_m` | 0.24 | **0.175** | 실측 (5.4) |
| `max_steer_angle_rad` | 0.5236 | **0.3054** | 실측 17.5° (5.4) |
| `perception_to_rear_axle_x_m` | 0.265 | **0.20** | 0.175 + 0.025 (5.4) |
| `nominal_control_dt_sec` | 0.10 | **0.45** | 실제 프레임 주기 |
| `command_watchdog_sec` | 0.5 | **1.5** | 0.455초 주기 대비 여유 (5.1) |
| `max_step_dt_sec` | 0.25 | **0.6** | 클립이 주기보다 커야 함 (5.2) |
| `sign_confirm_frames` | 3 | **2** | 3이면 1.36초 연속 검출 요구 → 안 걸림 (5.3) |
| `path_lost_stop_frames` | 10 | **2** | 10이면 4.55초 무경로 주행 (5.3) |
| `branch_*` / `crossing_*` / `fork_exit_off` | 3~8 | **1~2** | 벽시계 기준 근사 (5.3) |

노드 시작 로그에 해결된 값이 찍히므로 어느 차량을 몰고 있는지 즉시 확인할 수 있다:

```
planner_profile=real: wheelbase=0.175m, max_steer=0.3054rad,
rear_axle_offset=0.200m, watchdog=1.50s, max_step_dt=0.60s
```

> ⚠️ **프레임 카운트는 근사일 뿐 번역이 아니다.** 2.2 Hz에서는 1프레임이 이미 0.45초라
> 시뮬의 3프레임(0.10초) 필터를 재현할 방법이 없다. 근본 해법은 "N프레임 연속"을
> **시간 기반** 또는 **M중 N** 판정으로 바꾸는 것이다. 특히 `sign_confirm_frames`는
> 연속 검출을 요구하므로 YOLO가 한 프레임만 놓쳐도 리셋된다 — 후속 과제.

## 5.10 카메라 소스 캡스 (패치로 관리)

`v4l2src`의 소스 캡스에 width/height가 없어 **1920×1080으로 협상**되고 있었다.
보드가 매 프레임 1080p JPEG를 디코딩해 CPU로 320×180까지 축소한다.
C920e는 `image/jpeg 320×180 @30fps`를 **네이티브로 지원**한다.

캡처 파이프라인 CPU: **한 코어의 6.2% → 4.4%** (이득은 작다).
더 중요한 건 **종횡비 안전**이다 — 협상된 모드가 4:3이고 목표가 16:9면
`videoscale`이 비등방 압축을 하여 `fy ≠ fx`가 되는데, `metric_ipm.py`는
정사각 픽셀(`fy = fx`)을 가정하므로 IPM이 조용히 깨진다.
**7/8 bag이 320×240으로 찍힌 것이 이 증상이다.**

D-Racer-Kit은 주최측 저장소라 push할 수 없고 `init_workspace.sh`가 새로 clone하므로,
수정은 **팀 저장소의 `patches/camera-native-caps.patch`** 로 관리한다.
`init_workspace.sh`가 clone 직후 멱등적으로 적용한다.
