...
| `forced_turn` | 카메라 표지 무시 (`sign_ignored(forced=…)`) · **갈림 인지 ON** (테스트용) |
...
# Out 갈림 (라이브) — forced_turn 있으면 fork_on=1
...
**검토 결론: 대회 기본은 OR arm (표지 놓쳐도 capture로 판단).**

| 장면 | 트리거 | L/R |
|------|--------|-----|
| **Out 갈림** | **표지 인식 ∨ `out_fork_capture`** (`decide_out_fork_arm`, race: `require_sign=false`) | 표지가 rank 잠금 (0=좌, 1=우); 표지 miss → `default_out_branch_rank` |
| **In 탈출** | **`in_circle_fork_moment`만** (표지 불필요) | **1회 rising → 우(rank1)=원 유지** · **2회 rising → 좌(rank0)=탈출** |

```
OUT:  sign_window ──┐
      capture ──────┼──► enable_fork (OR) → yellow_alt/road_split follow → FORK_TURN
                    │
IN:   moment.hard ──(K debounce rising)──► pass1 keep_rank=1
                                         ► pass2 exit_rank=0 → EXIT_READY/EXIT
```

Strict AND (`require_sign=true` + `require_capture=true`)는 실험용: 표지만이면 평상 흰 차로에서 가짜 fork, capture만이면 표지 없는 먼 stretch에서 arm.  
대회 YAML은 **OR**: 표지가 보이면 그걸로 arm·L/R, 안 보이면 capture로 arm 후 기본 rank.  
