...
**LOCKED 2026-07-16 (trial #1 near):** black/cyan은 morph 전 **near-밴드 질량** CC만 유지.
비교용 trial #2(top_drop)는 `black_mode=top_drop` / `black_trials_*` 영상만.
...
BEV(Metric IPM)
  ...
  → morph open 5 / close 17 / 2 iter + 소구멍
...
| morph | open **5** / close **17** / 2회 · 채널 마스크는 open만 |
...
실차 검정 차로: V 하한을 낮춰 어두운 아스팔트를 살림 (상한 140 유지).  
**LOCKED 순서 (trial #1):** near-band **질량** CC 선택 → morph(open5/close17) → 하단 ego.  
...
좁은 고채도·고명도 cyan만 잡아 흰 카펫·흰 테이프 bleed를 피한다.  
`black_cyan_2`와 OR한 뒤 **near-ego CC만** road에 합침 (morph 전).
