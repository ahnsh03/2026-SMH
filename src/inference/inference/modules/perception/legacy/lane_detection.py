...
def make_dash_preview(
    ...
    *,
    ...
    if yellow_pts is None or getattr(yellow_pts, 'size', 0) == 0:
        yellow_pts = np.zeros(debug.bev.shape[:2], dtype=np.uint8)
    if yellow_conn is None or getattr(yellow_conn, 'size', 0) == 0:
        yellow_conn = yellow_pts.copy()
    if white_pts is None or getattr(white_pts, 'size', 0) == 0:
...
def render_mode_preview(mode: str, debug: LaneDebugFrame) -> np.ndarray:
    ...
    if mode == "yellow":
        ...
        overlay = base.copy()
        dash = debug.yellow_dash_points_bev
        if dash is not None and getattr(dash, 'size', 0):
            overlay[dash > 0] = (255, 255, 0)
        connected_mask = debug.yellow_connected_bev
        if connected_mask is not None and getattr(connected_mask, 'size', 0):
            connected = cv2.cvtColor(connected_mask, cv2.COLOR_GRAY2BGR)
